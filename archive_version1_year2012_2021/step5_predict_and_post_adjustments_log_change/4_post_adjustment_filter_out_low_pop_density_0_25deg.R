# --------------------------------- Task Summary --------------------------------- #
# # This file performs post-adjustment on the predicted 0.25-degree cell GDP values 
...
load("step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_0_25deg_with_prov_boundary.RData")
...
pred_0_25deg_with_prov_bound <- predict_data_results_0_25deg_with_prov_boundary %>%
                            dplyr::select(c(cell_id, subcell_id, subcell_id_0_25, id, iso, year, unit_gdp_af_sum_rescl, pred_GCP_share_0_25deg, pred_GCP_share_0_25deg_rescaled, pred_GCP_0_25deg, geom))  %>% 
                            left_join(pop)  %>% 
                            left_join(land_area)  %>% 
                            mutate(pop_density_km2 = ifelse(land_area_km2 == 0, 0, pop/land_area_km2)) %>% # Some small islands have population data, but the landcover data does not capture them. This issue can be ignored.
                            st_as_sf() %>% 
...
save(pred_0_25deg_with_prov_bound_postadjust_pop_dens_no_extra_adjust, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/pred_0_25deg_with_prov_bound_postadjust_pop_dens_no_extra_adjust.RData")

deg0_25_geometry <- read_sf("step5_predict_and_post_adjustments_log_change/outputs/country_0_25deg_intersected.gpkg")  %>%
...
save(pop_cell_0_25deg, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/pop_cell_0_25deg.RData")  
...
save(GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust.RData")

# also generate csv file, instead of giving geometry, give longitude and latitude of the bottom-left corner of each cell
# just_grid_0_25deg <- read_sf("/share/rossihansberglab/Nightlights_GDP/replication_packages/step3_obtain_cell_level_GDP_and_predictors_data/outputs/just_grid_0_25degree.gpkg")

# just_grid_0_25deg$longitude <- NA
# just_grid_0_25deg$latitude <- NA

# # Extract the bottom-left point for each cell
# for (i in 1:nrow(just_grid_0_25deg)) {
#     bbox <- st_bbox(just_grid_0_25deg[i, ])
#     just_grid_0_25deg$longitude[i] <- bbox$xmin
#     just_grid_0_25deg$latitude[i] <- bbox$ymin
# }

# just_grid_0_25deg_with_lon_lat <- just_grid_0_25deg  %>% as.data.frame()  %>% dplyr::select(-c(geom))
# write.csv(just_grid_0_25deg_with_lon_lat, file = "step5_predict_and_post_adjustments/outputs/just_grid_0_25deg_with_lon_lat.csv", row.names = FALSE)
...
GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust_csv <- GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust  %>% 
  ...
  as.data.frame()  %>% 
  dplyr::select(-c(geom))
write.csv(GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust_csv, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust.csv", row.names = FALSE)
...
GDPC_0_25deg_postadjust_pop_dens_0_01_adjust_csv <- GDPC_0_25deg_postadjust_pop_dens_0_01_adjust  %>% 
  ...
  as.data.frame()  %>% 
  dplyr::select(-c(geom))
write.csv(GDPC_0_25deg_postadjust_pop_dens_0_01_adjust_csv, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_0_01_adjust.csv", row.names = FALSE)
...
GDPC_0_25deg_postadjust_pop_dens_0_02_adjust_csv <- GDPC_0_25deg_postadjust_pop_dens_0_02_adjust  %>% 
  ...
  as.data.frame()  %>% 
  dplyr::select(-c(geom))
write.csv(GDPC_0_25deg_postadjust_pop_dens_0_02_adjust_csv, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_0_02_adjust.csv", row.names = FALSE)
...
GDPC_0_25deg_postadjust_pop_dens_0_05_adjust_csv <- GDPC_0_25deg_postadjust_pop_dens_0_05_adjust  %>% 
  ...
  as.data.frame()  %>% 
  dplyr::select(-c(geom))
write.csv(GDPC_0_25deg_postadjust_pop_dens_0_05_adjust_csv, file = "step5_predict_and_post_adjustments_log_change/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_0_05_adjust.csv", row.names = FALSE)
