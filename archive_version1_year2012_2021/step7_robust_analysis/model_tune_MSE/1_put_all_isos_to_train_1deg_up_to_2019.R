...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    rf_grid <- expand.grid(mtry = c(6,9,12,15,18,21),
    ...
    save(rf_workflow_final, file = "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/rf_workflow_final_1deg_up_to_2019.RData")
    toc()
  }
  ...
  rf_fit_1deg <- rf_fit
  save(rf_fit_1deg, file = "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/rf_fit_1deg_up_to_2019.RData")
  toc()
