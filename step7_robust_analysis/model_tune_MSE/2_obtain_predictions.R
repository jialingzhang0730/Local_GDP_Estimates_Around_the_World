# --------------------------------- Task Summary --------------------------------- #
# This file obtains predictions from the MSE-tuned model (trained in
...
pop <- land_pop_extracted_region_level_1deg %>%
  st_drop_geometry() %>%  # Remove the geometry column first
  filter(year <= 2022) %>% 
  as.data.frame() %>% 
...
rf_model_good <- rf_model9_good_grid_search_1deg_up_to_2019 # because the name of the df in "rf_model9_good_grid_search_1deg_up_to_2019.RData" is "rf_model9_good_grid_search_1deg", so don't make it wrong
