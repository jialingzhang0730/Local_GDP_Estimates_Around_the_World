...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    rf_grid <- expand.grid(mtry = c(26,28,29,30,32),
                           trees = c(1500),
                           min_n = c(1200,1500,1800,1900,2100,2200,2400,2700,3300,3600,3900,4300,4700))
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_0_25deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_0_25deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_0_25deg.RData")
