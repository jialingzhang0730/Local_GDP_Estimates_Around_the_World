...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    rf_grid <- expand.grid(mtry = c(34,36,37,39),
                           trees = c(1500),
                           min_n = c(1500,1700,1900,2100,2550,2750,2950,3000,3100,3300,3500,3800,4100,4500))
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_0_5deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_0_5deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step7_robust_analysis/model_tune_MSE/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_0_5deg.RData")
