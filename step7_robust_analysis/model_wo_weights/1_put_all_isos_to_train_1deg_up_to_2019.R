...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_1deg_up_to_2019/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_1deg_up_to_2019.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_1deg_up_to_2019.RData")
