...
# Task Summary:
#
# This file is to train the random forest model for 1 degree cell using all data, without weights
# ------------------------------------------------------------------------------------------------- #
...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    save(rf_workflow_final, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/rf_workflow_final_1deg.RData")
    toc()
  }
  ...
  rf_fit_1deg <- rf_fit
  save(rf_fit_1deg, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/rf_fit_1deg.RData")
  toc()
