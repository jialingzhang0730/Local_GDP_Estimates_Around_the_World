...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    rf_grid <- expand.grid(mtry = c(22,26,28,33,37,39,43),
                            trees = c(1600),
                            min_n = c(400,800,1300,1800,2100,2350,2700,3000,3300,3600,4000,4400,4800,5500))
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_1deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_1deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_1deg.RData")
    ...
    save(tuning_results_1deg, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/tuning_results_1deg.RData")
    ...
    write.csv(best_model_metrics, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_1deg.csv", row.names = FALSE)
...
save(rf_model9_good_grid_search_1deg, file = "step7_robust_analysis/model_wo_weights/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_1deg.RData")
toc()
