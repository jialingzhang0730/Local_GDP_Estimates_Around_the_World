# --------------------------------- Task Summary --------------------------------- #
# This file trains the 0.5-degree random forest model using data from 2012 to 2022, excluding all CO$_2$ emission predictors and their lags (Online Appendix Section 7.4).
# -------------------------------------------------------------------------------- #
...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  predictor_vars <- c("pop_total_share","pop_urban_share", "pop_cropland_share", "pop_other_share", "NPP_share",
                      ...
                      "lag_pop_urban_share", "lag_NPP_share", "lag_pop_cropland_share","lag_pop_other_share")
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_0_5deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_0_5deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_0_5deg.RData")
    ...
    save(tuning_results_0_5deg, file = "step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/tuning_results_0_5deg.RData")
    ...
    write.csv(best_model_metrics, file = "step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_0_5deg.csv", row.names = FALSE)
...
save(rf_model9_good_grid_search_0_5deg, file = "step7_robust_analysis/model_exclude_co2/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_0_5deg.RData")
toc()
