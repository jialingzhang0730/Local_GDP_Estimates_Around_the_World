# --------------------------------- Task Summary --------------------------------- #
# This file trains the 0.5-degree random forest model using data from 2012 to 2022, omitting the within-country GDP-share rescaling step (Online Appendix Section 8.3).
# -------------------------------------------------------------------------------- #
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      for (i in seq_along(df.cv$splits)) {
        ...
        # Save raw predictions once, then use for both rescaled and no-rescaling paths
        ...
        assessment_norescale <- assessment %>%
          ...
          mutate(prev_yr_pred_gdp_norescale = ifelse(year - 1 %in% year, pred_GCP_norescale[match(year - 1, year)], NA),
                 prev_yr_true_gdp = ifelse(year - 1 %in% year, GCP_0_5deg[match(year - 1, year)], NA)) %>%
          ungroup() %>%
          as.data.frame()
        ...
        # No-rescaling OOS R² metrics
        o_r2_levl_dped_norescale[i] <- overall_r2_levl(developed_norescale$GCP_0_5deg, developed_norescale$pred_GCP_norescale)
        o_r2_levl_dping_norescale[i] <- overall_r2_levl(developing_norescale$GCP_0_5deg, developing_norescale$pred_GCP_norescale)
        o_r2_levl_all_norescale[i] <- overall_r2_levl(assessment_norescale$GCP_0_5deg, assessment_norescale$pred_GCP_norescale)

        o_r2_chan_dped_norescale[i] <- overall_r2_chan(developed_norescale_ch$GCP_0_5deg, developed_norescale_ch$prev_yr_true_gdp, developed_norescale_ch$pred_GCP_norescale, developed_norescale_ch$prev_yr_pred_gdp_norescale)
        o_r2_chan_dping_norescale[i] <- overall_r2_chan(developing_norescale_ch$GCP_0_5deg, developing_norescale_ch$prev_yr_true_gdp, developing_norescale_ch$pred_GCP_norescale, developing_norescale_ch$prev_yr_pred_gdp_norescale)
        o_r2_chan_all_norescale[i] <- overall_r2_chan(assessment_norescale_ch$GCP_0_5deg, assessment_norescale_ch$prev_yr_true_gdp, assessment_norescale_ch$pred_GCP_norescale, assessment_norescale_ch$prev_yr_pred_gdp_norescale)
        # --- End no-rescaling OOS path ---
        ...
        mse_GDP_sh_dping_fit[i] <- MSE_GDP_sh(developing_fit$GCP_share_0_5deg, developing_fit$pred_GCP_sh)
        mse_GDP_sh_all_fit[i] <- MSE_GDP_sh(analysis_fit$GCP_share_0_5deg, analysis_fit$pred_GCP_sh)
        ...
        mse_levl_dping_fit[i] <- MSE_levl(developing_fit$GCP_0_5deg, developing_fit$pred_GCP)
        mse_levl_all_fit[i] <- MSE_levl(analysis_fit$GCP_0_5deg, analysis_fit$pred_GCP)

        # metric 3: MSE for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        mse_chan_dped_fit[i] <- MSE_chan(developed_fit_ch$GCP_0_5deg, developed_fit_ch$prev_yr_true_gdp, developed_fit_ch$pred_GCP, developed_fit_ch$prev_yr_pred_gdp)
        mse_chan_dping_fit[i] <- MSE_chan(developing_fit_ch$GCP_0_5deg, developing_fit_ch$prev_yr_true_gdp, developing_fit_ch$pred_GCP, developing_fit_ch$prev_yr_pred_gdp)
        mse_chan_all_fit[i] <- MSE_chan(analysis_fit_ch$GCP_0_5deg, analysis_fit_ch$prev_yr_true_gdp, analysis_fit_ch$pred_GCP, analysis_fit_ch$prev_yr_pred_gdp)
        ...
        o_r2_levl_dping_fit[i] <- overall_r2_levl(developing_fit$GCP_0_5deg, developing_fit$pred_GCP)
        o_r2_levl_all_fit[i] <- overall_r2_levl(analysis_fit$GCP_0_5deg, analysis_fit$pred_GCP)

        # metric 5: overall R2 for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        o_r2_chan_dped_fit[i] <- overall_r2_chan(developed_fit_ch$GCP_0_5deg, developed_fit_ch$prev_yr_true_gdp, developed_fit_ch$pred_GCP, developed_fit_ch$prev_yr_pred_gdp)
        o_r2_chan_dping_fit[i] <- overall_r2_chan(developing_fit_ch$GCP_0_5deg, developing_fit_ch$prev_yr_true_gdp, developing_fit_ch$pred_GCP, developing_fit_ch$prev_yr_pred_gdp)
        o_r2_chan_all_fit[i] <- overall_r2_chan(analysis_fit_ch$GCP_0_5deg, analysis_fit_ch$prev_yr_true_gdp, analysis_fit_ch$pred_GCP, analysis_fit_ch$prev_yr_pred_gdp)
        ...
        w_r2_levl_dping_fit[i] <- within_iso_r2_levl(developing_fit, GCP_0_5deg, pred_GCP)
        w_r2_levl_all_fit[i] <- within_iso_r2_levl(analysis_fit, GCP_0_5deg, pred_GCP)

        # metric 7: within-country R2 for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        w_r2_chan_dped_fit[i] <- within_iso_r2_chan(developed_fit_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)
        w_r2_chan_dping_fit[i] <- within_iso_r2_chan(developing_fit_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)
        w_r2_chan_all_fit[i] <- within_iso_r2_chan(analysis_fit_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)

        # Growth rate error metrics (fit)
        growth_met_dped_fit[[i]] <- growth_metrics(developed_fit_ch$GCP_0_5deg, developed_fit_ch$prev_yr_true_gdp,
                                                    developed_fit_ch$pred_GCP, developed_fit_ch$prev_yr_pred_gdp)
        growth_met_dping_fit[[i]] <- growth_metrics(developing_fit_ch$GCP_0_5deg, developing_fit_ch$prev_yr_true_gdp,
                                                     developing_fit_ch$pred_GCP, developing_fit_ch$prev_yr_pred_gdp)
        growth_met_all_fit[[i]] <- growth_metrics(analysis_fit_ch$GCP_0_5deg, analysis_fit_ch$prev_yr_true_gdp,
        ...
        mse_GDP_sh_dping[i] <- MSE_GDP_sh(developing$GCP_share_0_5deg, developing$pred_GCP_sh)
        mse_GDP_sh_all[i] <- MSE_GDP_sh(assessment_pred$GCP_share_0_5deg, assessment_pred$pred_GCP_sh)

        # metric 2: MSE for pred log(GDP) vs true log(GDP)
        mse_levl_dped[i] <- MSE_levl(developed$GCP_0_5deg, developed$pred_GCP)
        mse_levl_dping[i] <- MSE_levl(developing$GCP_0_5deg, developing$pred_GCP)
        mse_levl_all[i] <- MSE_levl(assessment_pred$GCP_0_5deg, assessment_pred$pred_GCP)

        # metric 3: MSE for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        mse_chan_dped[i] <- MSE_chan(developed_ch$GCP_0_5deg, developed_ch$prev_yr_true_gdp, developed_ch$pred_GCP, developed_ch$prev_yr_pred_gdp)
        mse_chan_dping[i] <- MSE_chan(developing_ch$GCP_0_5deg, developing_ch$prev_yr_true_gdp, developing_ch$pred_GCP, developing_ch$prev_yr_pred_gdp)
        mse_chan_all[i] <- MSE_chan(assessment_pred_ch$GCP_0_5deg, assessment_pred_ch$prev_yr_true_gdp, assessment_pred_ch$pred_GCP, assessment_pred_ch$prev_yr_pred_gdp)

        # metric 4: overall R2 for pred log(GDP) vs true log(GDP)
        o_r2_levl_dped[i] <- overall_r2_levl(developed$GCP_0_5deg, developed$pred_GCP)
        o_r2_levl_dping[i] <- overall_r2_levl(developing$GCP_0_5deg, developing$pred_GCP)
        o_r2_levl_all[i] <- overall_r2_levl(assessment_pred$GCP_0_5deg, assessment_pred$pred_GCP)

        # metric 5: overall R2 for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        o_r2_chan_dped[i] <- overall_r2_chan(developed_ch$GCP_0_5deg, developed_ch$prev_yr_true_gdp, developed_ch$pred_GCP, developed_ch$prev_yr_pred_gdp)
        o_r2_chan_dping[i] <- overall_r2_chan(developing_ch$GCP_0_5deg, developing_ch$prev_yr_true_gdp, developing_ch$pred_GCP, developing_ch$prev_yr_pred_gdp)
        o_r2_chan_all[i] <- overall_r2_chan(assessment_pred_ch$GCP_0_5deg, assessment_pred_ch$prev_yr_true_gdp, assessment_pred_ch$pred_GCP, assessment_pred_ch$prev_yr_pred_gdp)

        # metric 6: within-country R2 for pred log(GDP) vs true log(GDP)
        w_r2_levl_dped[i] <- within_iso_r2_levl(developed, GCP_0_5deg, pred_GCP)
        w_r2_levl_dping[i] <- within_iso_r2_levl(developing, GCP_0_5deg, pred_GCP)
        w_r2_levl_all[i] <- within_iso_r2_levl(assessment_pred, GCP_0_5deg, pred_GCP)

        # metric 7: within-country R2 for log(pred GDP_t) - log(pred GDP_t-1)  vs log(true GDP_t) - log(true GDP_t-1)
        w_r2_chan_dped[i] <- within_iso_r2_chan(developed_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)
        w_r2_chan_dping[i] <- within_iso_r2_chan(developing_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)
        w_r2_chan_all[i] <- within_iso_r2_chan(assessment_pred_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP, prev_yr_pred_gdp)

        # --- No-log R2 metrics (after rescaling, raw GDP levels) ---
        o_r2_levl_dped_nolog[i] <- overall_r2_levl_nolog(developed$GCP_0_5deg, developed$pred_GCP)
        o_r2_levl_dping_nolog[i] <- overall_r2_levl_nolog(developing$GCP_0_5deg, developing$pred_GCP)
        o_r2_levl_all_nolog[i] <- overall_r2_levl_nolog(assessment_pred$GCP_0_5deg, assessment_pred$pred_GCP)

        o_r2_chan_dped_nolog[i] <- overall_r2_chan_nolog(developed_ch$GCP_0_5deg, developed_ch$prev_yr_true_gdp, developed_ch$pred_GCP, developed_ch$prev_yr_pred_gdp)
        o_r2_chan_dping_nolog[i] <- overall_r2_chan_nolog(developing_ch$GCP_0_5deg, developing_ch$prev_yr_true_gdp, developing_ch$pred_GCP, developing_ch$prev_yr_pred_gdp)
        o_r2_chan_all_nolog[i] <- overall_r2_chan_nolog(assessment_pred_ch$GCP_0_5deg, assessment_pred_ch$prev_yr_true_gdp, assessment_pred_ch$pred_GCP, assessment_pred_ch$prev_yr_pred_gdp)

        # --- MAPE metrics (after rescaling) ---
        mape_dped[i] <- mape_fn(developed$GCP_0_5deg, developed$pred_GCP)
        mape_dping[i] <- mape_fn(developing$GCP_0_5deg, developing$pred_GCP)
        mape_all[i] <- mape_fn(assessment_pred$GCP_0_5deg, assessment_pred$pred_GCP)

        # --- Growth rate error metrics (OOS) ---
        growth_met_dped[[i]] <- growth_metrics(developed_ch$GCP_0_5deg, developed_ch$prev_yr_true_gdp,
                                                developed_ch$pred_GCP, developed_ch$prev_yr_pred_gdp)
        growth_met_dping[[i]] <- growth_metrics(developing_ch$GCP_0_5deg, developing_ch$prev_yr_true_gdp,
                                                 developing_ch$pred_GCP, developing_ch$prev_yr_pred_gdp)
        growth_met_all[[i]] <- growth_metrics(assessment_pred_ch$GCP_0_5deg, assessment_pred_ch$prev_yr_true_gdp,
        ...
        assessment_pred_decile <- assessment_pred %>%
          mutate(decile = ntile(GCP_0_5deg, 10))
        ...
        for (d in 1:10) {
          ...
          if (nrow(dec_data) > 1 & nrow(dec_data_ch) > 1) {
            decile_metrics[[paste0("fold_", i, "_decile_", d)]] <- data.frame(
              ...
              o_r2_levl = overall_r2_levl(dec_data$GCP_0_5deg, dec_data$pred_GCP),
              o_r2_chan = overall_r2_chan(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp, dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp),
              o_r2_levl_nolog = overall_r2_levl_nolog(dec_data$GCP_0_5deg, dec_data$pred_GCP),
              o_r2_chan_nolog = overall_r2_chan_nolog(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp, dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp),
              mean_log_error = mean(log(dec_data$pred_GCP) - log(dec_data$GCP_0_5deg)),
              mean_abs_log_error = mean(abs(log(dec_data$pred_GCP) - log(dec_data$GCP_0_5deg))),
              mean_pct_error = mean((dec_data$pred_GCP - dec_data$GCP_0_5deg) / dec_data$GCP_0_5deg) * 100,
              mape = mean(abs(dec_data$pred_GCP - dec_data$GCP_0_5deg) / dec_data$GCP_0_5deg) * 100,
              mean_true_GCP = mean(dec_data$GCP_0_5deg),
              mean_pred_GCP = mean(dec_data$pred_GCP),
              mean_true_growth = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
                                                 dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp)$mean_true_growth,
              mean_pred_growth = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
                                                 dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp)$mean_pred_growth,
              mean_growth_error = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
                                                  dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp)$mean_growth_error,
              mean_abs_growth_error = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
                                                      dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp)$mean_abs_growth_error,
              mean_log_change_error = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
                                                      dec_data_ch$pred_GCP, dec_data_ch$prev_yr_pred_gdp)$mean_log_change_error,
              mean_abs_log_change_error = growth_metrics(dec_data_ch$GCP_0_5deg, dec_data_ch$prev_yr_true_gdp,
        ...
        # --- Within-iso R2 (no rescaling) ---
        w_r2_levl_dped_norescale[i] <- within_iso_r2_levl(developed_norescale, GCP_0_5deg, pred_GCP_norescale)
        w_r2_levl_dping_norescale[i] <- within_iso_r2_levl(developing_norescale, GCP_0_5deg, pred_GCP_norescale)
        w_r2_levl_all_norescale[i] <- within_iso_r2_levl(assessment_norescale, GCP_0_5deg, pred_GCP_norescale)

        w_r2_chan_dped_norescale[i] <- within_iso_r2_chan(developed_norescale_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP_norescale, prev_yr_pred_gdp_norescale)
        w_r2_chan_dping_norescale[i] <- within_iso_r2_chan(developing_norescale_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP_norescale, prev_yr_pred_gdp_norescale)
        w_r2_chan_all_norescale[i] <- within_iso_r2_chan(assessment_norescale_ch, GCP_0_5deg, prev_yr_true_gdp, pred_GCP_norescale, prev_yr_pred_gdp_norescale)
      ...
      write.csv(decile_summary, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/decile_metrics_0_5deg_no_rescaling.csv", row.names = FALSE)
      ...
      write.csv(metrics_df, file = sprintf("step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_0_5deg_no_rescaling/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_0_5deg_no_rescaling.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_0_5deg_no_rescaling.RData")
      ...
      cat("No-Rescaling Within R2 (Level) log(GDP)\n")
      ...
      cat("No-Rescaling Within R2 (Change) log(GDP)\n")
    ...
    save(tuning_results_0_5deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/tuning_results_0_5deg_no_rescaling.RData")
    ...
    write.csv(best_model_metrics, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_0_5deg_no_rescaling.csv", row.names = FALSE)
...
save(rf_model9_good_grid_search_0_5deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_0_5deg_no_rescaling.RData")
toc()
