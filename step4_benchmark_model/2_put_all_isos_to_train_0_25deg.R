  ...
  if(tune_par){
    ...
    for (i in seq_along(df.cv$splits)) {
      ...
      oos_predictions_list[[i]] <- oos_i %>%
        select(cell_id, subcell_id, subcell_id_0_25, iso, year, cv_fold,
               oos_predicted_GCP, GCP_0_25deg,
    ...
    write.csv(oos_all,
              "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/oos_cv_predictions_0_25deg.csv",
              row.names = FALSE)
    cat(sprintf("Saved %d cell-level OOS predictions to oos_cv_predictions_0_25deg.csv\n", nrow(oos_all)))
