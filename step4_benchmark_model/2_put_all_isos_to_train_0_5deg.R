...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_0_5deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_0_5deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_0_5deg.RData")
    ...
    save(tuning_results_0_5deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/tuning_results_0_5deg.RData")
    ...
    write.csv(best_model_metrics, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_0_5deg.csv", row.names = FALSE)

    # ---- Export cell-level OOS predictions using best hyperparameters ---- #
    # These predictions hold out entire countries (group CV by ISO), avoiding
    # spatial/temporal contamination. Used by downstream robustness scripts
    # (border discontinuity, agriculture, CO2/transport corridor) and
    # uncertainty error maps.

    cat("\nGenerating cell-level OOS predictions with best hyperparameters...\n")
    cat(sprintf("  Best params: mtry=%d, trees=%d, min_n=%d\n",
                param_final$mtry, param_final$trees, param_final$min_n))

    oos_predictions_list <- list()

    for (i in seq_along(df.cv$splits)) {
      cat(sprintf("  Fold %d/%d...\n", i, length(df.cv$splits)))

      analysis_i   <- as.data.frame(analysis(df.cv$splits[[i]]))
      assessment_i <- as.data.frame(assessment(df.cv$splits[[i]]))

      fit_i <- rand_forest(mtry = param_final$mtry,
                           trees = param_final$trees,
                           min_n = param_final$min_n) %>%
        set_engine("ranger", importance = "impurity", verbose = TRUE, seed = 1234567) %>%
        set_mode("regression") %>%
        fit(formula, data = analysis_i, weights = import_weight)

      oos_i <- assessment_i %>%
        mutate(pred_GCP_sh_ns = predict(fit_i, assessment_i)$.pred) %>%
        mutate(pred_GCP_sh_ns = ifelse(floor(pop_total) == 0, 0, pred_GCP_sh_ns)) %>%
        group_by(iso, year) %>%
        mutate(pred_GCP_sh = pred_GCP_sh_ns / sum(pred_GCP_sh_ns)) %>%
        ungroup() %>%
        mutate(oos_predicted_GCP = pred_GCP_sh * state_total_GDP,
               cv_fold = i)

      oos_predictions_list[[i]] <- oos_i %>%
        select(cell_id, subcell_id, iso, year, cv_fold,
               oos_predicted_GCP, GCP_0_5deg,
               pop_total_share, cropland_share, NPP_share,
               CO2_non_org_tspt_share, CO2_bio_tspt_share,
               state_total_GDP)

      rm(fit_i, oos_i, analysis_i, assessment_i); gc()
    }

    oos_all <- bind_rows(oos_predictions_list)
    write.csv(oos_all,
              "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/oos_cv_predictions_0_5deg.csv",
              row.names = FALSE)
    cat(sprintf("Saved %d cell-level OOS predictions to oos_cv_predictions_0_5deg.csv\n", nrow(oos_all)))

    rm(oos_predictions_list, oos_all); gc()
    # ---- End OOS prediction export ---- #
...
save(rf_model9_good_grid_search_0_5deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_0_5deg.RData")
toc()
