...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      write.csv(metrics_df, file = sprintf("step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/detailed_metric_1deg/detail_metrics_%s_%s_%s.csv", params$mtry, params$trees, params$min_n), row.names = FALSE)
      ...
      write.csv(datapoint_counts_df, "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_metrics_fold_N_1deg.csv", row.names = FALSE)

      # Save the variable importance scores
      save(var_importance, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/var_imptc_score_1deg.RData")
    ...
    save(tuning_results_1deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/tuning_results_1deg.RData")
    ...
    write.csv(best_model_metrics, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_1deg.csv", row.names = FALSE)
    ...
    for (i in seq_along(df.cv$splits)) {
      ...
      oos_predictions_list[[i]] <- oos_i %>%
        select(cell_id, iso, year, cv_fold,
               oos_predicted_GCP, GCP_1deg,
    ...
    write.csv(oos_all,
              "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/oos_cv_predictions_1deg.csv",
              row.names = FALSE)
    cat(sprintf("Saved %d cell-level OOS predictions to oos_cv_predictions_1deg.csv\n", nrow(oos_all)))
...
save(rf_model9_good_grid_search_1deg, file = "step4_benchmark_model/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_1deg.RData")
toc()
