...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    rf_grid <- expand.grid(mtry = c(6,9,12,15,18,21,24,27,30,33),
                           trees = c(400,700,1000,1200,1500),
                           min_n = c(300,600,800,900,1000,1100,1200,1300,1400,1500,1650,1800,2100,2325))
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      for (i in seq_along(df.cv$splits)) {
        ...
        # Add this debugging:
        if(i == 2) {  # Only check Fold 2 since that's where the NA occurs
          
          # Check for zero sums before rescaling
          zero_sum_check <- assessment_pred %>%
            group_by(iso, year) %>%
            summarise(
              sum_pred_ns = sum(pred_GCP_sh_ns),
              n_cells = n(),
              .groups = 'drop'
            ) %>%
            filter(sum_pred_ns == 0 | is.na(sum_pred_ns))
          
          if(nrow(zero_sum_check) > 0) {
            print("WARNING: Zero sum predictions found in Fold 2:")
            print(zero_sum_check)
          }
          
          # Check for NaN after rescaling
          nan_check <- assessment_pred %>%
            filter(is.na(pred_GCP_sh) | is.infinite(pred_GCP_sh))
          
          if(nrow(nan_check) > 0) {
            print("WARNING: NaN/Inf values after rescaling in Fold 2:")
            print(table(nan_check$iso, nan_check$year))
          }
        }
