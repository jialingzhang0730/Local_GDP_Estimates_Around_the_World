...
model_9_1 <- predict_data_results_1deg_without_prov_boundary  %>% 
  ...
  dplyr::select(-c("country_total_GDP", "national_population"))  %>% 
  as.data.frame()  %>% 
  distinct(cell_id, iso, year, .keep_all = TRUE) %>%
  group_by(cell_id, iso) %>% 
  mutate(growth_91 = log(pred_GCP_1deg_no_prov_bound[year == 2022]) - log(pred_GCP_1deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
model_9_5 <- predict_data_results_1deg_without_prov_boundary_model_up_to_2019  %>% 
  ...
  distinct(cell_id, iso, year, .keep_all = TRUE) %>%
  group_by(cell_id, iso) %>% 
  mutate(growth_95 = log(pred_GCP_1deg_no_prov_bound[year == 2022]) - log(pred_GCP_1deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
calculate_r_squared = 1 - (sum((df_complete$growth_91 - df_complete$growth_95)^2) / 
                             sum((df_complete$growth_91 - mean(df_complete$growth_91))^2))
calculate_r_squared
...
model_9_1 <- predict_data_results_0_5deg_without_prov_boundary  %>% 
  ...
  mutate(growth_91 = log(pred_GCP_0_5deg_no_prov_bound[year == 2022]) - log(pred_GCP_0_5deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
model_9_5 <- predict_data_results_0_5deg_without_prov_boundary_model_up_to_2019  %>% 
  ...
  distinct(cell_id, subcell_id, iso, year, .keep_all = TRUE) %>%
  group_by(cell_id, subcell_id, iso) %>% 
  mutate(growth_95 = log(pred_GCP_0_5deg_no_prov_bound[year == 2022]) - log(pred_GCP_0_5deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
model_9_1 <- predict_data_results_0_25deg_without_prov_boundary  %>% 
  ...
  mutate(growth_91 = log(pred_GCP_0_25deg_no_prov_bound[year == 2022]) - log(pred_GCP_0_25deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
model_9_5 <- predict_data_results_0_25deg_without_prov_boundary_model_up_to_2019  %>% 
  ...
  distinct(cell_id, subcell_id, subcell_id_0_25, iso, year, .keep_all = TRUE) %>%
  group_by(cell_id, subcell_id, subcell_id_0_25, iso) %>% 
  mutate(growth_95 = log(pred_GCP_0_25deg_no_prov_bound[year == 2022]) - log(pred_GCP_0_25deg_no_prov_bound[year == 2021])) %>% 
  ungroup() %>% 
...
ggsave("step5_predict_and_post_adjustments/outputs/analyze_diff_models/compare_log_change_all_deg.png", plot = combined_plot, width = 22, height = 14, bg = "white")
