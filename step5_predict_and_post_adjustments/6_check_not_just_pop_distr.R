...
# 1deg
load("step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density/GDPC_1deg_postadjust_pop_dens_no_extra_adjust.RData")
...
# 0.5deg
load("step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density/GDPC_0_5deg_postadjust_pop_dens_no_extra_adjust.RData")
...
# 0.25deg
load("step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density/GDPC_0_25deg_postadjust_pop_dens_no_extra_adjust.RData")
...
for (i in c(4)){
  ...
  p1 <- ggplot(pred_1deg, aes(x = log_pop, y = log_GCP)) +
    ...
    labs(subtitle = paste("1deg cell predictions: all years"),
         ...
         y = "log GDP (const 2021 billion USD)") +
...
for (i in c(4)){
  ...
  p2 <- ggplot(pred_0_5deg, aes(x = log_pop, y = log_GCP)) +
    ...
    geom_line(aes(y = fitted), color = "blue", size = 1) +
    geom_abline(intercept = coef(poly0)[1], slope = 1, linetype = "dashed", color = "#BE1508", size = 1) +
    labs(subtitle = paste("0.5 deg cell predictions: all years"),
         x = "log Pop Count (million)",
         y = "log GDP (const 2021 billion USD)") +
    scale_x_continuous(limits = x_limits) +
    scale_y_continuous(limits = y_limits) +         
...
for (i in c(4)){
  ...
  p3 <- ggplot(pred_0_25deg, aes(x = log_pop, y = log_GCP)) +
    ...
    geom_line(aes(y = fitted), color = "blue", size = 1) +
    geom_abline(intercept = coef(poly0)[1], slope = 1, linetype = "dashed", color = "#BE1508", size = 1) +
    labs(subtitle = paste("0.25 deg cell predictions: all years"),
         x = "log Pop Count (million)",
         y = "log GDP (const 2021 billion USD)") +
    scale_x_continuous(limits = x_limits) +
    scale_y_continuous(limits = y_limits) +  
