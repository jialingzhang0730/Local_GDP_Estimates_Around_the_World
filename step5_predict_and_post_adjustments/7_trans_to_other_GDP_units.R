...
#       draw some plots, the unit is in constant 2021 USD. Now we will rescale the data to other units:
#       current USD, current PPP-adjusted international$, constant 2021 PPP-adjusted international$ 
# ------------------------------------------------------------------------------------------------- #
...
files_gdp <- list.files(path = "step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density", pattern = "^GDPC.*(\\.csv|_adjust\\.RData)$", full.names = TRUE)
...
rescale_result <- lapply(files_gdp, function(file_path){
  ...
  if (file_ext == "csv") {
    result_final <- result
    new_file_path <- file.path("step5_predict_and_post_adjustments/outputs/final_output_dataset", paste0("final_", sub("\\.[^\\.]+$", ".csv", basename(file_path))))
    write.csv(result_final, new_file_path, row.names = FALSE)
    
  } else if (file_ext == "RData") {
    result_final <- result %>% st_as_sf()
    new_file_path <- file.path("step5_predict_and_post_adjustments/outputs/final_output_dataset", paste0("final_", basename(file_path)))
