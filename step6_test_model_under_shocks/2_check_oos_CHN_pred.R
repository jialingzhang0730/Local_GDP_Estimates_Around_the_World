...
load("step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density/pred_1deg_with_prov_bound_postadjust_pop_dens_no_extra_adjust.RData")
...
plot1 <- ggplot() +
  ...
  scale_x_continuous(breaks = seq(2012, 2022, 1)) +  # Add x-axis breaks from 2012-2022
  labs(
    y = "1deg Cell GDP",
    x = "Year"
  ) +
...
wuhan_data <- china_df %>% 
  filter(cell_id == 21535) %>%
  arrange(year) %>%
  mutate(
    log_change_pred = log(pred_GCP_1deg) - log(lag(pred_GCP_1deg)),
    log_change_actual = log(GCP_1deg) - log(lag(GCP_1deg))
  ) %>%
  filter(!is.na(log_change_pred))  # Remove first year since no lag available

plot1 <- ggplot() +
  geom_point(data = wuhan_data, 
             aes(x = year, y = log_change_pred, color = "Predicted"), 
             size = 2, shape = 19, alpha = 0.7) +
  geom_line(data = wuhan_data, 
            aes(x = year, y = log_change_pred, color = "Predicted"), 
            size = 1.2) +
  geom_point(data = wuhan_data, 
             aes(x = year, y = log_change_actual, color = "Actual"), 
             size = 2, shape = 17, alpha = 0.7) +
  geom_line(data = wuhan_data, 
            aes(x = year, y = log_change_actual, color = "Actual"), 
            size = 1.2) +
  ...
  scale_color_manual(values = c(name = NULL, "Predicted" = "#BE1508", "Actual" = "#7030A0")) +
  scale_x_continuous(breaks = seq(2013, 2022, 1)) +  # Set x-axis breaks from 2013-2022 (2012 is filtered out)
  labs(
    y = "Log Change in 1 deg Cell GDP",
    x = "Year"
  ) +
  theme_minimal(base_size = 14) +
  theme(
...
ggsave("step6_test_model_under_shocks/outputs/CHN_test/wuhan_time_series_log_change.pdf", 
       plot = plot1,
       bg = "white", width = 10)
...
# read the predicted GDP
load("step5_predict_and_post_adjustments/outputs/predict_data_results_postadjust_pop_density/pred_1deg_with_prov_bound_postadjust_pop_dens_no_extra_adjust_upto_2019.RData")
...
ggsave("step6_test_model_under_shocks/outputs/CHN_test/log_level_change_r2_use_model_upto_2019.png", plot = combined_plot, width = 18, height = 12, bg = "white")