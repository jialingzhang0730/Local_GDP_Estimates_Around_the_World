# --------------------------------- Task Summary --------------------------------- #
# This file trains the 0.25-degree random forest model using data from 2012 to 2019.
# -------------------------------------------------------------------------------- #
...
train_rf <- function(data_full, df.cv = folds, name = "RF", tune_par = T){
  ...
  if(tune_par){
    ...
    tune_hyperparameters <- function(params, data_full, df.cv) {
      ...
      for (i in seq_along(df.cv$splits)) {
        ...
        mse_developed[i] <- calculate_mse(developed$GCP_share_0_25deg, developed$pred_GCP_share_0_25deg)
        mse_developing[i] <- calculate_mse(developing$GCP_share_0_25deg, developing$pred_GCP_share_0_25deg)
        ...
        r2_developed[i] <- calculate_r2(developed$GCP_0_25deg, developed$pred_GCP_0_25deg)
        r2_developing[i] <- calculate_r2(developing$GCP_0_25deg, developing$pred_GCP_0_25deg)
        ...
        chan_r2_developed[i] <- calculate_chan_r2(developed$GCP_0_25deg, developed$prev_year_true, developed$pred_GCP_0_25deg, developed$prev_year_pred)
        chan_r2_developing[i] <- calculate_chan_r2(developing$GCP_0_25deg, developing$prev_year_true, developing$pred_GCP_0_25deg, developing$prev_year_pred)
    ...
    save(tuning_results_0_25deg, file = "step4_train_and_tune_log_change/outputs/model9_tuning/put_all_isos_to_train/tuning_results_0_25deg_up_to_2019.RData")
    ...
    write.csv(best_model_metrics, file = "step4_train_and_tune_log_change/outputs/model9_tuning/put_all_isos_to_train/best_model_metrics_0_25deg_up_to_2019.csv", row.names = FALSE)
    ...
    save(rf_workflow_final, file = "step4_train_and_tune_log_change/outputs/model9_tuning/put_all_isos_to_train/rf_workflow_final_0_25deg_up_to_2019.RData")
    toc()
  }
  ...
  save(rf_fit_0_25deg, file = "step4_train_and_tune_log_change/outputs/model9_tuning/put_all_isos_to_train/rf_fit_0_25deg_up_to_2019.RData")
  toc()
...
save(rf_model9_good_grid_search_0_25deg, file = "step4_train_and_tune_log_change/outputs/model9_tuning/put_all_isos_to_train/rf_model9_good_grid_search_0_25deg_up_to_2019.RData")
toc()
