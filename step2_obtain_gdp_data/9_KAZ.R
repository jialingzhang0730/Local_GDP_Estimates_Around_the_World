# --------------------------------- Task Summary --------------------------------- #
# This file retrieves regional GDP data for Kazakhstan for all available years
# and processes the corresponding geometries.
#
# NOTE: Kazakhstan's regional boundaries changed between 2008 and 2022:
#       1. Starting from 2018, Ontustik Kazakhstan is separated into Shymkent city + Turkistan
#       2. Starting from 2021, Abay is separated from Shygys Kazakhstan
#       3. Starting from 2021, Zhetisu is separated from Аlmaty
#       4. Starting from 2021, Ulytau is separated from Кaragandy
# -------------------------------------------------------------------------------- #
...
ost <- read_excel("version2_year2012_2022/step2_obtain_gdp_data/inputs/gdp_data/regional/KAZ/1. Gross regional product.xlsx", sheet = "2008-2023", skip = 2, n_max = 22)  %>% 
       ...
       mutate(`2008` = ifelse(is.na(`2008`), sum(`2008`[2:3]), `2008`),
            ...
            `2022` = ifelse(is.na(`2022`), sum(`2022`[2:3]), `2022`))  %>% # Before 2018, "Ontustik Kazakhstan" has values. After 2018, the values are the sum of "Shymkent city" and "Turkistan"
...
as <- read_excel("version2_year2012_2022/step2_obtain_gdp_data/inputs/gdp_data/regional/KAZ/1. Gross regional product.xlsx", sheet = "2008-2023", skip = 2, n_max = 22)  %>% 
       ...
       mutate(`2008` = ifelse(admin_2_name == "Shygys Kazakhstan", sum(`2008`[1:2]), `2008`),
            ...
            `2022` = ifelse(admin_2_name == "Shygys Kazakhstan", sum(`2022`[1:2]), `2022`)) %>% # This replaces "Shygys Kazakhstan"'s values with sum of "Abay" and "Shygys Kazakhstan"
            # This only changes "Shygys Kazakhstan"'s values since 2021, because before 2021, "Abay"'s values are 0
...
za <- read_excel("version2_year2012_2022/step2_obtain_gdp_data/inputs/gdp_data/regional/KAZ/1. Gross regional product.xlsx", sheet = "2008-2023", skip = 2, n_max = 22)  %>% 
       ...
       mutate(`2008` = ifelse(admin_2_name == "Аlmaty", sum(`2008`[1:2]), `2008`),
            ...
            `2022` = ifelse(admin_2_name == "Аlmaty", sum(`2022`[1:2]), `2022`)) %>% # This replaces "Аlmaty"'s values with sum of "Zhetisu" and "Аlmaty"
...
uk <- read_excel("version2_year2012_2022/step2_obtain_gdp_data/inputs/gdp_data/regional/KAZ/1. Gross regional product.xlsx", sheet = "2008-2023", skip = 2, n_max = 22)  %>% 
       ...
       mutate(`2008` = ifelse(admin_2_name == "Кaragandy", sum(`2008`[1:2]), `2008`),
            ...
            `2022` = ifelse(admin_2_name == "Кaragandy", sum(`2022`[1:2]), `2022`)) %>% # This replaces "Кaragandy"'s values with sum of "Ulytau" and "Кaragandy"
            # This only changes "Кaragandy"'s values since 2021, because before 2021, "Ulytau"'s values are 0
