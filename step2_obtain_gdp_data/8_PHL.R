# --------------------------------- Task Summary --------------------------------- #
# This file retrieves regional GDP data for Philippines for all available years
...
PHL_regional_rgdp <- read_excel("version2_year2012_2022/step2_obtain_gdp_data/inputs/gdp_data/regional/PHL/GRDP_Reg_2018PSNA_2000-2023.xlsx", skip = 9, n_max = 18)  %>% 
...
write.csv(PHL_regional_rgdp, "version2_year2012_2022/step2_obtain_gdp_data/temp/phl_gdp_clean.csv", row.names = F)
...
write.csv(training_df, "version2_year2012_2022/step2_obtain_gdp_data/temp/phl_training_data.csv", row.names = F)

# ------------------------------------------------- #
# Create shapefiles

PHL_regional_sf <- read_sf("version2_year2012_2022/step1_obtain_gis_data/outputs/CGAZ_ADM1_without_large_waters.gpkg") %>%  
...
PHL_national_sf <- PHL_regional_sf %>% 
  mutate(name = "Philippines") %>% 
  group_by(name, iso) %>% 
  summarize(geom = st_union(geom),
            admin_unit = 1, .groups = "drop")

st_write(PHL_regional_sf, "version2_year2012_2022/step2_obtain_gdp_data/temp/phl_admin_2.gpkg", append = F)
st_write(PHL_national_sf, "version2_year2012_2022/step2_obtain_gdp_data/temp/phl_admin_1.gpkg", append = F)
