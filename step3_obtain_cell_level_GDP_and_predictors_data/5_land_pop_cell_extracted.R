...
population_files <- list.files("step3_obtain_cell_level_GDP_and_predictors_data/inputs/population", full.names = T)[1:11] #choose years only after 2012
...
# simplified_poly <- read_sf("step3_obtain_cell_level_GDP_and_predictors_data/outputs/world_province_0_1deg_with_cellid.gpkg")  %>% 
#                     select(-c("fid_2")) 
...
#   extract <- cbind(simplified_poly, exact_extract(r, simplified_poly, 'sum')) %>% 
#               rename(pop = exact_extract.r..simplified_poly...sum..)
#   save(extract, file = paste0("step3_obtain_cell_level_GDP_and_predictors_data/outputs/land_pop_year_sep/land_pop_extracted_0_1deg", as.numeric(str_extract(filename, "\\d{4}")), ".RData"))
...
# land_pop_full <- NULL 
# for (year in years){
#           load(paste0("step3_obtain_cell_level_GDP_and_predictors_data/outputs/land_pop_year_sep/land_pop_extracted_0_1deg", year, ".RData"))
#           extract <- extract %>%
#               mutate(year = as.integer(year))
...
#           }
# }
# land_pop_extracted_region_level_0_1deg <- land_pop_full  %>% 
#                       replace_na(list(pop = 0)) 

# save(land_pop_extracted_region_level_0_1deg, file = "step3_obtain_cell_level_GDP_and_predictors_data/outputs/land_pop_extracted_region_level_0_1deg.RData")
