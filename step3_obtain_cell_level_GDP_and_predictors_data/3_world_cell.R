...
world_model8_poly <- bind_rows(poly_nontraining, USA, rest_train_iso_county_bound)  %>% 
  mutate(id = ifelse(iso == "Ala", "Ala", id)) # make alaska's id also ti be "Ala"
...
fulfill_gdp <- read.csv("step2_obtain_gdp_data/temp/national_gdp_const_2021_USD.csv")  %>% # recall that we use national_gdp_const_2017_USD 
  ...
  mutate(id = iso, 
         rescale_level = 1,
         unit_gdp_af_sum_rescl = rgdp_2021_USD,
         country_total_GDP = rgdp_2021_USD,
         national_population = population) %>% 
  dplyr::select(-c(Country, rgdp_2021_USD, population))
